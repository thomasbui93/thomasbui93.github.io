...
export const CareerPiece = (props) => (<div className={style.container}>
  <Col justifyContent="center">
    <Row className={style.headline}>
      ...
      <div className={style.companyBox}>
        <Col>
          ...
          <SM>{props.start} - {props.end}</SM>
        </Col>
        <Col>
          <Row>
            {props.skills.map((skill, index) => <div className={style.skillIcon}>
              <Avatar size="extrasmall" key={index}>
                <img src={require(`../../images/skills/${skill.toLowerCase()}.${ isSvg(skill) ? "svg": "png"}`)} alt={skill}/>
              </Avatar>
            </div>
            )}
          </Row>
    ...
    <Row>
      ...
      <Col>
        {props.details.map((detail, index) => <Row>
...
export const JobPiece = (props) => (<Col className={style.jobPiece}>
  <MD isBold>{props.title}</MD>
  <SM >{props.start} - {props.end}</SM>
  <UnorderedList>
    {props.duties.map((duty, index) => <UnorderedList.Item key={index}>{duty}</UnorderedList.Item>)}
  </UnorderedList>
</Col>)
